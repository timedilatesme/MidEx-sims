...
def get_rubin_cadence_dict(filters, ra=None, dec=None, starting_time_offset = 0):
    if ra == None:
        ra = 360*np.random.random()
        dec = -90 + 90*np.random.random()
    each_band_obs = get_rubin_cadence(ra, dec, columns=['filter', 'observationStartMJD'])
    cadence_dict = {}
    for filter in filters:
        cadence_dict[filter] = np.array(each_band_obs['observationStartMJD'][filter]) + starting_time_offset
    return cadence_dict